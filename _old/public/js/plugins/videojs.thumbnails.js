(function () {
    ...
    const parseImageLink = function (imglocation) {
        let lsrc, clip, hashindex, hashstring;
   ...
   */
    videojs.registerPlugin(`thumbnails`, function (options) {
        let div, settings, img, player, progressControl, duration, moveListener, moveCancel, thumbTrack;
        ...
        player.on(`medialoaded`, function (event) {
            ...
            const numtracks = player.textTracks().length;
            if (numtracks === 0) {
		  ...
		  while (i < cnum) {
            ...
            const el3 = player.el_.querySelector(`.vjs-thumb-tooltip`);
            if (el3 !== null) el3.parentNode.removeChild(el3);
            const el2 = player.el_.querySelector(`.vjs-thumb-image`);
            if (el2 !== null) el2.parentNode.removeChild(el2);
            const el1 = player.el_.querySelector(`.vjs-thumbnail-holder`);
		  ...
		  duration = player.duration();
            ...
            const ppr = this.el_.querySelector(`.vjs-play-progress`);
            const ttp = ppr.querySelector(`.vjs-time-tooltip`);
            videojs.dom.addClass(ttp, `vjs-abs-hidden`);
            const mtp = progressControl.el().querySelector(`.vjs-mouse-display`);
            mtp.style.opacity = 0;

            moveListener = function (event) {
		  const msds = this.el_.querySelector(`.vjs-mouse-display`);
		  let left = Number(msds.style.left.replace(/px$/, ``));

		  const percent = left / progressControl.el().offsetWidth;
		  const mouseTime = percent * duration;
		  const tlp = mtp.querySelector(`.vjs-time-tooltip`);
		  tooltip.innerHTML = tlp.innerHTML;

		  // Now check which of the cues applies
		  const cnum = thumbTrack && thumbTrack.cues.length;
		  i = 0; let is_slide = false;
		  while (i < cnum) {
                    const ccue = thumbTrack.cues[i];
		  ...
		  const right = progressControl.el().offsetWidth;
		  const holef = player.el_.querySelector(`.vjs-progress-holder`).offsetLeft;
		  const halfWidth2 = halfWidth - holef;
