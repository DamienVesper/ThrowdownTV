    ...
    function getAds (player) {
        return {
            ...
            isLive: function isLive () {
                const somePlayer = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : player;
...
*/
    ...
    const cancelEvent = function cancelEvent (player, event) {
    ...
    const prefixEvent = function prefixEvent (player, prefix, event) {
    ...
    const handlePlaying = function handlePlaying (player, event) {
    ...
    const handleEnded = function handleEnded (player, event) {
        if (player.ads.isInAdMode()) {
    ...
    const handleLoadEvent = function handleLoadEvent (player, event) {
        // Initial event
    ...
    const handlePlay = function handlePlay (player, event) {
...
*/
    ...
    function initializeContentupdate (player) {
        ...
        const checkSrc = function checkSrc () {
            if (!player.ads.inAdBreak()) {
    ...
    const empty$1 = (Object.freeze || Object)({
        default: empty
    });

    const minDoc = (empty$1 && empty) || empty$1;

    const topLevel = typeof commonjsGlobal !== `undefined`
        ? commonjsGlobal
...
*/
    ...
    const uriEncodeIfNeeded = function uriEncodeIfNeeded (value, uriEncode) {
        if (uriEncode) {
    ...
    const customFields = function customFields (mediainfo, macros, customFieldsName) {
    ...
    function adMacroReplacement (string, uriEncode, customMacros) {
        ...
        string = string.replace(/{([^}=]+)=([^}]+)}/g, (match, name, defaultVal) => {
        ...
        // Page variables
        string = string.replace(/{pageVariable\.([^}]+)}/g, (match, name) => {
            let value = void 0;
            let context = window_1;
...
**/
    cueTextTracks.processMetadataTracks = function (player, processMetadataTrack) {
        ...
        const setModeAndProcess = function setModeAndProcess (track) {
...
**/
    const cueAlreadySeen = function cueAlreadySeen (player, cueId) {
...
* @param cueId The Id associated with a cue
**/
    const setCueAlreadySeen = function setCueAlreadySeen (player, cueId) {
 ...
 */
  ...
  videojsReference.middleware && videojsReference.middleware.TERMINATOR) {
    ...
    const playMiddleware = obj.playMiddleware;
    const isMiddlewareMediatorSupported$1 = obj.isMiddlewareMediatorSupported;
 ...
 */
    ...
    const hasAdsPlugin = function hasAdsPlugin () {
 ...
 */
    ...
    const AdState = (function (_State) {
        inherits(AdState, _State);

        function AdState (player) {
            classCallCheck(this, AdState);

            const _this = possibleConstructorReturn(this, _State.call(this, player));
   ...
   */
    ...
    const ContentState = (function (_State) {
...
*/
    ...
    let tryToResumeTimeout_ = void 0;
 ...
 */
    function restorePlayerSnapshot (player, callback) {
        ...
        let trackSnapshot = void 0;
        const restoreTracks = function restoreTracks () {
        ...
        const resume = function resume () {
            ...
            if (videojs.browser.IS_IOS && player.ads.isLive(player)) {
                ...
                const _playPromise = player.play();

                if (_playPromise && _playPromise.catch) {
                    _playPromise.catch((error) => {
        ...
        // This only happens if the content video element was reused for ad playback.
        const tryToResume = function tryToResume () {
 ...
 */
    ...
    const obj$1 = { start: start, end: end };
 ...
 */
   ...
   */
        ...
        Preroll.prototype.init = function init (player, adsReady, shouldResumeToContent) {
            ...
            this._timeout = player.setTimeout(() => {
   ...
   */
    ...
    const Midroll = (function (_AdState) {
   ...
   */
    ...
    const Postroll = (function (_AdState) {
   ...
   */
        ...
        Postroll.prototype.init = function init (player) {
            ...
            if (!player.ads.nopostroll_) {
                ...
                this._postrollTimeout = player.setTimeout(() => {
                    player.trigger(`adtimeout`);
                }, timeout);
   ...
   */
	  ...
	  player.ads.settings.prerollTimeout = 0;
	  console.log(`no postroll2`);
 ...
 */
   ...
   */
    ...
    const StitchedAdRoll = (function (_AdState) {
...
*/
    ...
    const isMiddlewareMediatorSupported = obj.isMiddlewareMediatorSupported;
    ...
    const contribAdsPlugin = function contribAdsPlugin (options) {
        ...
        const videoEvents = VIDEO_EVENTS.concat([`firstplay`, `loadedalldata`, `playing`]);
        ...
        const shouldDisableTracks = function shouldDisableTracks () {
   ...
   */
        const textTrackChangeHandler = function textTrackChangeHandler () {
        ...
        player.on([`play`, `playing`, `ended`, `adsready`, `adscanceled`, `adskip`, `adserror`, `adtimeout`, `adended`, `ads-ad-started`, `contentchanged`, `dispose`, `contentresumed`, `readyforpostroll`, `nopreroll`, `nopostroll`], (e) => {
