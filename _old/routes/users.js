...
router.post(`/register`, (req, res) => {
    ...
    const bannedUsernames = [`users`, `dashboard`, `register`, `login`, `tos`, `browse`, `logout`, `follow`, `unfollow`, `following`, `streams`, `demo`, `getvip`, `api`, `throwdown`, `throwdowntv`, `officialthrowdown`, `officialthrowdowntv`];
...
router.post(`/newpassword/:resetlink`, (req, res) => {
    ...
    if (errors.length > 0) {
        ...
        User.findOne({ reset_link: reset_link }).then(useraccount => {
            if (useraccount) {
                jwt.verify(reset_link, config.jwtToken_resetpassword, (error, decodedData) => {
                    if (error) {
                        req.flash(
                            `error_msg`,
