...
const run = async (message: string, args: string[], chatter: Chatter, chatUsers: Chatter[]) => {
    ...
    else if (!userToTimeoutExists) return chatter.socket.emit(`commandMessage`, `That user does not exist!`);
    else if (chatter.username === userToTimeout) return chatter.socket.emit(`commandMessage`, `You cannot time yourself out!`);
    else if (streamer.channel.timeouts.includes(userToTimeout)) return chatter.socket.emit(`commandMessage`, `That user is already barred from your channel's chat!`);
    ...
    streamer.save(() => chatter.socket.emit(`commandMessage`, `${userToTimeout} has been timed out for 5 minutes.`));

    const users = chatUsers.filter(user => user.channel === chatter.channel && user.username !== chatter.username);
    for (const user of users) user.socket.emit(`commandMessage`, `${userToTimeout} was timed out for 5 minutes by a moderator.`);
    ...
    setTimeout(() => {
        ...
        streamer.save(() => {
            for (const user of users) user.socket.emit(`commandMessage`, `${userToTimeout} was automatically unmuted.`);
            chatter.socket.emit(`commandMessage`, `${userToTimeout} was automatically unmuted.`);
        });
    }, 3e5);
};
