...
module.exports.run = async (message, args, chatter, chatUsers) => {
    ...
    if (!userToUnbanExists) return chatter.emit(`commandMessage`, `That user does not exist!`);
    ...
    for (const user of users) user.emit(`broadcastMessage`, `${userToUnban} was banned by a moderator.`);
};
