    ...
    if (user) {
      ...
      if (req.isAuthenticated()) {
          ...
          axios.get('http://eu01.throwdown.tv/api/streams/live/' + user.stream_key, { auth: { username: 'admin', password: 'loltdtv2021' } })
            .then(function (response) {
              if (response.data.isLive) {
                renderStream("https://eu01.throwdown.tv/live/" + user.stream_key + "/index.mpd", "application/dash+xml", followbutton, followoption, req.params.username.toLowerCase(), "ONLINE", "lime", response.data.viewers, user.chat_token)
              } else {
                axios.get('http://us01.throwdown.tv/api/streams/live/' + user.stream_key, { auth: { username: 'admin', password: 'loltdtv2021' } })
                  .then(function (response) {
                    if (response.data.isLive) {
                      renderStream("https://us01.throwdown.tv/live/" + user.stream_key + "/index.mpd", "application/dash+xml", followbutton, followoption, req.params.username.toLowerCase(), "ONLINE", "lime", response.data.viewers, user.chat_token)
                    } else {
        ...
        axios.get('http://eu01.throwdown.tv/api/streams/live/' + user.stream_key, { auth: { username: 'admin', password: 'loltdtv2021' } })
          .then(function (response) {
            if (response.data.isLive) {
              renderStream("https://eu01.throwdown.tv/live/" + user.stream_key + "/index.mpd", "application/dash+xml", "Follow", "follow", req.params.username.toLowerCase(), "ONLINE", "lime", response.data.viewers, user.chat_token)
            } else {
              axios.get('http://us01.throwdown.tv/api/streams/live/' + user.stream_key, { auth: { username: 'admin', password: 'loltdtv2021' } })
                .then(function (response) {
                  if (response.data.isLive) {
                    renderStream("https://us01.throwdown.tv/live/" + user.stream_key + "/index.mpd", "application/dash+xml", "Follow", "follow", req.params.username.toLowerCase(), "ONLINE", "lime", response.data.viewers, user.chat_token)
                  } else {
                    renderStream("throwdown.mp4", "video/mp4", "Follow", "follow", req.params.username.toLowerCase(), "OFFLINE", "red", response.data.viewers)
                  }            
                })
